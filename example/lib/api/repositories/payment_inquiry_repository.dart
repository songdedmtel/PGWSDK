...
import 'package:pgw_sdk_example/api/models/payment_inquiry_response.dart';
import 'package:pgw_sdk_example/api/repositories/constant_repository.dart';

class PaymentInquiryRepository implements IRepository {
  String? _payload;

  @override
  Future<PaymentInquiryResponse?> execute() async {
    if (_payload == null) throw 'Please set token before execute';

    final response = await http.post(
      Uri.parse('https://sandbox-pgw.2c2p.com/payment/4.1/PaymentInquiry'),
      headers: {
    ...
    try {
      ...
      // Return response
      final inquiryResponse = PaymentInquiryResponse.fromMap(jwt.payload);
  ...
  PaymentInquiryRepository setRequest({
    ...
    final jsonRequest = {
      ...
      "invoiceNo": invoiceNo,
      "locale": ConstantRepository.LOCALE,
    };
    // Log
